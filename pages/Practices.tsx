    ...
    <div className="min-h-screen bg-white dark:bg-black">
      ...
      <header className="relative py-28 bg-black overflow-hidden border-b border-accent-gold/20">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-accent-gold/20 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2"></div>
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="max-w-3xl mx-auto text-xl text-slate-400 leading-relaxed font-light">
            Apex Advocates & Solicitors offers a comprehensive suite of legal services designed to protect and promote the commercial interests of our global and local clientele.
          </p>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
          {PRACTICE_AREAS.map((area, index) => (
            <div 
              key={area.id} 
              id={area.id}
              className={`flex flex-col lg:flex-row gap-16 items-center scroll-mt-32 ${index % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}
            >
              {/* Text Side */}
              <div className="w-full lg:w-1/2">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h2 className="text-3xl font-bold text-black dark:text-white uppercase tracking-tight">{area.title}</h2>
                </div>
                <p className="text-lg text-slate-600 dark:text-slate-400 leading-relaxed mb-10">
                  {area.description} Our firm leverages a decade of experience to provide "outside the box" resolutions for complex legal challenges within this domain. We ensure that every strategy is meticulously tailored to the specific needs of our clients.
                </p>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12">
                   {area.services.map((service, idx) => (
                     <div key={idx} className="flex items-start gap-3 group">
                       <span className="material-icons text-accent-gold text-sm mt-1 transition-transform group-hover:scale-125">verified</span>
                       <span className="text-sm font-semibold text-black dark:text-slate-300">{service}</span>
                     </div>
                   ))}
                </div>
              ...
              <div className="w-full lg:w-1/2">
                <div className="relative group">
                  <div className="absolute -inset-1 bg-accent-gold/20 rounded-[2.5rem] blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
                  <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden border border-accent-gold/10">
                    <img 
                      src={`https://images.unsplash.com/${index % 2 === 0 ? 'photo-1589829545856-d10d557cf95f' : 'photo-1497366216548-37526070297c'}?q=80&w=2070&auto=format&fit=crop`} 
                      alt={area.title} 
      ...
      <section className="py-24 bg-slate-50 dark:bg-secondary-grey/5 border-y border-accent-gold/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-accent-gold font-bold uppercase tracking-widest text-sm mb-4">Our Methodology</h2>
          <h3 className="text-4xl font-extrabold text-black dark:text-white mb-16">The Apex Advantage</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <div key={i} className="p-8 bg-white dark:bg-black rounded-3xl border border-accent-gold/5 shadow-xl transition-all hover:border-accent-gold/30 group">
                <div className="w-16 h-16 bg-accent-gold/10 rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:bg-accent-gold transition-colors">
                  <span className="material-icons text-accent-gold text-3xl group-hover:text-black">{step.icon}</span>
                </div>
                <h4 className="text-xl font-bold mb-4 text-black dark:text-white uppercase tracking-tight">{step.title}</h4>
      ...
      <section className="bg-black py-24 relative overflow-hidden">
        <div className="absolute inset-0 opacity-5 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-accent-gold rounded-full blur-[160px]"></div>
        </div>
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          <h2 className="text-4xl font-black text-white mb-8 uppercase tracking-tight">Need a Bespoke Legal Strategy?</h2>
